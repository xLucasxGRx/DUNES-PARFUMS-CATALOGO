/**
 * Dunes Parfums - Tests unitarios para la respuesta garantizada del Botón Verde (#btn-confirmar-whatsapp)
 */
const { test, beforeEach } = require('node:test');
const assert = require('node:assert/strict');
const fs = require('fs');

globalThis.window = globalThis;

globalThis.location = {
    search: '',
    href: 'http://localhost/carrito.html',
    pathname: '/carrito.html'
};

globalThis.matchMedia = () => ({
    matches: false,
    addEventListener: () => {},
    removeEventListener: () => {}
});

let mockElements = {};
let elementsById = {};
globalThis.document = {
    addEventListener: () => {},
    querySelector: (sel) => mockElements[sel] || null,
    querySelectorAll: (sel) => mockElements[sel] || [],
    getElementById: (id) => elementsById[id] || null,
    createElement: (tag) => {
        let _id = '';
        const el = {
            get id() { return _id; },
            set id(v) { _id = v; elementsById[v] = el; },
            className: '',
            style: {},
            classList: {
                add: (c) => { el.className += ' ' + c; },
                remove: (c) => { el.className = (el.className || '').replace(c, '').trim(); },
                contains: (c) => (el.className || '').includes(c)
            },
            setAttribute: (k, v) => { if (k === 'id') el.id = v; },
            getAttribute: () => null,
            appendChild: () => {},
            addEventListener: () => {},
            querySelector: () => null
        };
        return el;
    },
    body: {
        classList: {
            add: (c) => mockElements['body_classes'].add(c),
            remove: (c) => mockElements['body_classes'].delete(c),
            contains: (c) => mockElements['body_classes'].has(c)
        },
        appendChild: () => {}
    }
};

globalThis.carritoModulo = {
    obtenerCarrito: () => [{ id: 'p1', nombre: 'Test', precioUnitario: 100, cantidad: 1 }]
};

eval(fs.readFileSync('js/interfaz.js', 'utf8'));

beforeEach(() => {
    mockElements = {
        'body_classes': new Set()
    };
    elementsById = {};
});

test('BotonVerde - validarDatosEntrega NUNCA asigna disabled=true al botón', () => {
    const btn = document.createElement('button');
    btn.id = 'btn-confirmar-whatsapp';
    btn.disabled = false;
    elementsById['btn-confirmar-whatsapp'] = btn;

    selectedDeliveryType = null; // Incompleto
    validarDatosEntrega();

    assert.equal(btn.disabled, false, 'El botón verde NUNCA debe estar deshabilitado');
});

test('BotonVerde - handleConfirmarPedido ejecuta la acción y devuelve el paso incompleto cuando faltan datos', () => {
    seleccionarTipoEntrega(null);
    const paso = obtenerPrimerPasoIncompleto();
    assert.ok(paso);
    assert.equal(paso.tipo, 'modalidad');
    assert.equal(paso.mensaje, 'Selecciona primero cómo deseas recibir tu pedido.');
});
